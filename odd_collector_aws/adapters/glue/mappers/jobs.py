...
GLUE_JOB_STATUSES = {
    ...
    "RUNNING": JobRunStatus.UNKNOWN,
    "STOPPING": JobRunStatus.UNKNOWN,
    "STOPPED": JobRunStatus.ABORTED,
    "SUCCEEDED": JobRunStatus.SUCCESS,
    "FAILED": JobRunStatus.FAILED,
    "ERROR": JobRunStatus.FAILED,
    "TIMEOUT": JobRunStatus.FAILED,
}
...
def map_glue_job_run(
    raw_job_run_data: Dict[str, Any], mapper_args: Dict[str, Any]
) -> DataEntity:
    status = GLUE_JOB_STATUSES.get(
        raw_job_run_data["JobRunState"], JobRunStatus.UNKNOWN
    )
    oddrn_generator: GlueGenerator = mapper_args["oddrn_generator"]
    return DataEntity(
        ...
        metadata=[
            metadata_extractor.extract_transformer_run_metadata(raw_job_run_data)
        ],
