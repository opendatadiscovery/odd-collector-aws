from odd_collector_aws.adapters.sagemaker.domain.artifact import create_artifact, Image, Model, Dataset


def test_create_artifact():
    image_artifact = {'name': 'SageMaker.ImageUri',
    ...
    assert artifact.uri == '1111.dkr.ecr.us-east-1.amazonaws.com/predict-flight-delay/components/preprocessing-serving:debug'
    assert isinstance(artifact, Image)

    model_artifact = {'name': 'SageMaker.ModelArtifact',
    ...
    assert artifact.uri == 's3://sagemaker-flight-delay-prediction-demo/pipelines-1111/output/model.tar.gz'
    assert isinstance(artifact, Model)

    input_artifact = {'name': 'input',
