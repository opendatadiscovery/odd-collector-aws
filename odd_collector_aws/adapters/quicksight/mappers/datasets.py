...
def map_quicksight_dataset(
    ...
    for k in raw_dataset_data["PhysicalTableMap"]:
        for j in raw_dataset_data["PhysicalTableMap"][k]:
            source_id = raw_dataset_data["PhysicalTableMap"][k][j][
                "DataSourceArn"
            ].split("/")[-1]
            source_info = __describe_data_source(
                quicksight_client, source_id, account_id
            )
            source_type = source_info["Type"]
            oddrn = ""
            if source_type == "S3":
                source_bucket = source_info["DataSourceParameters"]["S3Parameters"][
                    "ManifestFileLocation"
                ]["Bucket"]
                source_key = source_info["DataSourceParameters"]["S3Parameters"][
                    "ManifestFileLocation"
                ]["Key"]
                ...
                athena_oddrn = AthenaGenerator(
                    cloud_settings={"account": account_id, "region": region_name},
                    catalogs=raw_dataset_data["PhysicalTableMap"][k]["RelationalTable"][
                        "Catalog"
                    ],
                    databases=raw_dataset_data["PhysicalTableMap"][k][
                        "RelationalTable"
                    ]["Schema"],
                    tables=raw_dataset_data["PhysicalTableMap"][k]["RelationalTable"][
                        "Name"
                    ],
                )
                ...
                host = source_info["DataSourceParameters"]["PostgreSqlParameters"][
                    "Host"
                ]
                port = source_info["DataSourceParameters"]["PostgreSqlParameters"][
                    "Port"
                ]
                database = source_info["DataSourceParameters"]["PostgreSqlParameters"][
                    "Database"
                ]

                postgres_oddrn = PostgresqlGenerator(
                    host_settings=f"{host}:{port}", databases=database
                )
                ...
                database = source_info["DataSourceParameters"]["MySqlParameters"][
                    "Database"
                ]
                ...
                host = source_info["DataSourceParameters"]["SnowflakeParameters"][
                    "Host"
                ]
                database = source_info["DataSourceParameters"]["SnowflakeParameters"][
                    "Database"
                ]
                warehouse = source_info["DataSourceParameters"]["SnowflakeParameters"][
                    "Warehouse"
                ]

                snowflake_oddrn = SnowflakeGenerator(
                    host_settings=host, warehouses=warehouse, databases=database
                )
                oddrn = snowflake_oddrn.get_oddrn_by_path("databases")

            raw_dataset_data["DataTransformer"].append(
                {"oddrn": oddrn, "metadata": source_info}
            )
    return DataEntity(
        ...
        dataset=DataSet(parent_oddrn=None, rows_number=0, field_list=list(columns)),
        data_transformer=DataTransformer(
            inputs=[
                transformer["oddrn"]
                for transformer in raw_dataset_data["DataTransformer"]
            ],
            outputs=[],
        ),
    )
...
def __describe_data_source(client, data_source_id: str, account_id: str):
    raw_data_sources: Dict[str, Any] = client.describe_data_source(
